...
Writen by: https://github.com/MilesGrey
"""
import os
...
def earth_mover_distance(**kwargs: dict) -> Callable:
    """
    Wrapper for earth_mover distance for unified interface with self-guided earth mover distance
    loss.

    """

    def _earth_mover_distance(
        y_true: K.placeholder, y_pred: K.placeholder
    ) -> K.placeholder:
        return K.mean(
            tf.reduce_mean(
                tf.square(tf.cumsum(y_true, axis=-1) - tf.cumsum(y_pred, axis=-1)),
                axis=-1,
            )
        )
        ...
        if epoch == 4:
            ...
            self_guided_emd_loss = tf.reduce_mean(
                tf.convert_to_tensor(
                    self.self_guided_emd_loss_history, dtype=tf.float32
                )
            )
...
class GroundDistanceManager(Callback):
    ...
    def _estimate_distances(self) -> K.placeholder:
        normalized_features = self.epoch_class_features / tf.reduce_sum(
            self.epoch_class_features, axis=1, keepdims=True
        )
        ...
        for i in range(self.class_length):
            estimated_distances.append(
                tf.norm(tensor=centroids - centroids[i], ord="euclidean", axis=-1)
            )
        return tf.stack(estimated_distances)

    def _calculate_ground_distances(
        self, estimated_distances: K.placeholder
    ) -> K.placeholder:
    ...
    def _save_ground_distance_matrix(self, epoch: int) -> None:
        np.save(file=str(self.file_path) + f"/{epoch}", arr=self.ground_distance_matrix)
...
def self_guided_earth_mover_distance(
    model, ground_distance_sensitivity: float, ground_distance_bias: float
) -> Callable:
    ...
    def _self_guided_earth_mover_distance(
        ...
        cross_entropy_loss = categorical_crossentropy(y_true=y_true, y_pred=y_pred)
        if model.emd_weight_head_start.emd_weight == 0:
            if model.emd_weight_head_start.epoch == 3:
                ...
                model.emd_weight_head_start.cross_entropy_loss_history.append(
                    cross_entropy_loss
                )
            ...
            return (
                cross_entropy_loss
                + model.emd_weight_head_start.emd_weight * self_guided_emd_loss
            )
