...
def main():
    ...
    train_generator = rpn_data.DataGenerator(300, dataset_train, config, shuffle=False,
                                             phantom_output=True)

    os.makedirs(OUTPUT_FOLDER, exist_ok=True)
    is_info_saved = False
    for idx, ((images, gt_masks, rpn_match, rpn_bbox, gt_class_ids), _) in tqdm.tqdm(enumerate(
            train_generator), desc="Generating processed data"):
        if not is_info_saved:
            with open(os.path.join(OUTPUT_FOLDER, 'data.json'), 'w+') as f:
                json.dump({"total_matches": rpn_match.shape[1],
                           "whole_batch": int(SAVE_WHOLE_BATCH)}, f)
            is_info_saved = True
        ...
        if SAVE_WHOLE_BATCH:
            zarr.save(os.path.join(folder, f"image.zarr"), images * 255)
            zarr.save(os.path.join(folder, f"mask.zarr"), gt_masks)
            zarr.save(os.path.join(folder, f"matches.zarr"), rpn_match)
            np.save(os.path.join(folder, f"bboxes.npy"), rpn_bbox)
            np.save(os.path.join(folder, f"gt_class.npy"), gt_class_ids)
        else:
            zarr.save(os.path.join(folder, f"image.zarr"), images[0] * 255)
            zarr.save(os.path.join(folder, f"mask.zarr"), gt_masks[0])
            zarr.save(os.path.join(folder, f"matches.zarr"), rpn_match[0])
            np.save(os.path.join(folder, f"bboxes.npy"), rpn_bbox[0])
            np.save(os.path.join(folder, f"gt_class.npy"), gt_class_ids[0])
