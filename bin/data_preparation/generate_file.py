...
def hook(images: np.array, augmenter: object, *args: list, **kwargs: dict):
...
def get_contour_precedence(contour: Iterable[np.array], cols: int) -> np.array:
...
def main():
    ...
    while generation < TO_GENERATE:
        ...
        img, improved_mask, regions_augmented = normalize_dataset.normalize_img_mask(
            img, mask
        )

        bboxes_json[generation] = {
            "regions": regions_augmented,
            "cell_class": cells_type,
        }
