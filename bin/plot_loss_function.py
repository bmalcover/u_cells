...
from u_rpn.losses import bce, segmentation
...
def plot_loss(name_fn: str, loss_fn: Callable) -> None:
    ...
    train_generator = rpn_data.DataGenerator(
        50, dataset_train, config, shuffle=False, phantom_output=True
    )
    ...
    for i in tqdm(
        range(0, INPUT_SIZE[0] - STEP_IMPROVE, STEP_IMPROVE), desc="Horizontal axis"
    ):
        for j in tqdm(
            range(0, INPUT_SIZE[1] - STEP_IMPROVE, STEP_IMPROVE), desc="Vertical axis"
        ):
            for z in range(0, INPUT_SIZE[2] - STEP_IMPROVE, int(STEP_IMPROVE // 2)):
                random_2_perf[
                    i : i + STEP_IMPROVE, j : j + STEP_IMPROVE, z : z + STEP_IMPROVE
                ] = masks[0][
                    i : i + STEP_IMPROVE, j : j + STEP_IMPROVE, z : z + STEP_IMPROVE
                ]
                random_2_zero[
...
def main() -> None:
    functions = {"WBCE": bce.WeightedBCE(), "Dice RPN": segmentation.dice_rpn}
